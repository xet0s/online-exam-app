@extends('layouts.app', ['title' => 'Derslik Ekle - Sınav Dağıtım'])
...
<div class="max-w-2xl mx-auto">
    <div class="relative z-10">
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-extrabold tracking-tight text-slate-900 mt-2">Yeni Derslik Ekle</h1>
            <p class="text-sm text-slate-500 mt-1">Sınav dağıtımında kullanılmak üzere yeni derslik kapasite bilgilerini girin.</p>
        </div>
        ...
        <div class="glass p-8 rounded-none shadow-sm bg-white border border-slate-200">
            <form action="{{ route('classrooms.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input id="capacity" name="capacity" type="number" required min="1" value="{{ old('capacity') }}"
                ...
                @if(auth()->user()->isDean() || auth()->user()->isAdmin())
                    <div>
                        ...
                        <p class="text-[11px] text-slate-500 mt-1.5">Boş bırakılırsa tüm fakülte sınavlarında ortak kullanılabilir.</p>
